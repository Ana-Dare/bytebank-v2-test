...
describe('Rotas', () => {
  ...
  test('Deve renderizar a localização da rota atual', () => {
    ...
    expect(localizacaoAtual);
  });

  test('Deve renderizar a tela 404', () => {
    const rota = '/extrato';
    render(
      <MemoryRouter initialEntries={[rota]}>
        <AppRoutes />
    ...
    const pagianErro = screen.getByTestId('pagina-404');
    expect(pagianErro).toContainHTML('<h1>Ops! Não encontramos a página</h1>');
  });
});
